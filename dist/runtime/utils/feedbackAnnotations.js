...
export function getAnnotationBounds(annotation, measureText) {
...
export function getHandlePositions(annotation, bounds) {
  if (annotation.kind === "arrow") {
    return {
...
export function hitTestHandle(point, annotation, bounds, handleSize = HANDLE_SIZE) {
  const handles = getHandlePositions(annotation, bounds);
  const half = handleSize / 2 + 2;
  for (const [id, position] of Object.entries(handles)) {
    if (Math.abs(point.x - position.x) <= half && Math.abs(point.y - position.y) <= half) {
      return id;
    }
  }
  return null;
}
export function hitTestAnnotation(point, annotations, measureText) {
...
export function resizeAnnotation(annotation, handle, point, canvasWidth) {
  if (annotation.kind === "arrow") {
    if (handle === "start") {
...
export function drawAnnotation(ctx, annotation) {
  if (annotation.kind === "path") {
    ...
    ctx.strokeStyle = annotation.tool === "highlighter" ? withAlpha(annotation.color, 0.35) : annotation.color;
    ctx.beginPath();
...
export function drawSelectionOverlay(ctx, annotation, measureText) {
  const bounds = getAnnotationBounds(annotation, measureText);
  ctx.save();
